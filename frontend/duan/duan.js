    ...
    fetch("http://localhost:3000/project", requestOptions)
      ...
      .then(result => {
        if (result.statusCode === 200 && result.data) {
          ...
          console.error("Không thể lấy danh sách dự án:", result.message);
          callback(null);
        }
      })
      .catch(error => {
    ...
    fetch(`http://localhost:3000/project/${id}`, requestOptions)
      ...
      .then(result => {
        if (result.statusCode === 200 && result.data) {
          ...
          console.error("Không thể lấy thông tin dự án:", result.message);
          callback(null);
        }
      })
      .catch(error => {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (saveProjectBtn) {
    saveProjectBtn.addEventListener('click', addNewProject);
  ...
  function fetchProjects() {
    ...
    fetch("http://localhost:3000/project", requestOptions)
      ...
      .then(result => {
        if (result.statusCode === 200 && result.data) {
          console.log("Dữ liệu dự án:", result.data);
          populateTable(result.data);
        } else {
          console.error("Không thể lấy dữ liệu dự án:", result.message);
          if (tableBody) {
            tableBody.innerHTML = `<tr><td colspan="7" class="text-center text-danger">Không thể lấy dữ liệu dự án: ${result.message || 'Vui lòng thử lại sau'}</td></tr>`;
          }
        }
      })
      .catch(error => {
        ...
        if (tableBody) {
          tableBody.innerHTML = '<tr><td colspan="7" class="text-center text-danger">Lỗi kết nối đến máy chủ. Vui lòng thử lại sau.</td></tr>';
        }
      });
  }
  ...
  function populateTable(projects) {
    ...
    projects.forEach(project => {
      ...
      const formattedCreatedAt = `${String(createdAt.getDate()).padStart(2, '0')}/${String(createdAt.getMonth() + 1).padStart(2, '0')}/${createdAt.getFullYear()}`;
      ...
      row.innerHTML = `
        ...
        <td>${project.client_id || project.user_id || project.user?.username || 'N/A'}</td>
        <td>${formattedCreatedAt}</td>
        <td class="text-center">
          <button type="button" class="btn btn-info btn-sm me-1" data-bs-toggle="modal" data-bs-target="#viewModal-${project.id}" title="Xem chi tiết">
            <i class="bi bi-eye"></i>
          </button>
          <button type="button" class="btn btn-warning btn-sm me-1" data-bs-toggle="modal" data-bs-target="#editModal-${project.id}" title="Chỉnh sửa">
            <i class="bi bi-pencil-square"></i>
          </button>
          <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteModal-${project.id}" title="Xóa">
      ...
      // Tạo các modal cho mỗi dự án
      createProjectModals(project);
    });
    
    // Khởi tạo DataTable sau khi thêm dữ liệu
    const dataTable = document.querySelector('table.table');
    if (dataTable) {
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="editForm-${project.id}">
              ...
              <div class="mb-3">
                <label for="user_id-${project.id}" class="form-label">Người phụ trách (User)</label>
                <select class="form-select" id="user_id-${project.id}" required>
                  <option value="" disabled>-- Chọn người phụ trách --</option>
                  <!-- Danh sách users sẽ được nạp từ API -->
                </select>
              </div>
            </form>
          </div>
