...
window.storyAPI = {
  ...
  getAllStories: function(callback) {
    ...
    fetch('http://localhost:3000/stories', requestOptions)
      ...
      .then(result => {
        // API trả về mảng dữ liệu trực tiếp
        console.log('Stories data:', result);
        if (Array.isArray(result)) {
          callback(result);
        } else {
          console.error('Không thể lấy danh sách user story: Dữ liệu không đúng định dạng');
          callback(null);
        }
      })
      .catch(error => {
  ...
  getStoryById: function(id, callback) {
    ...
    fetch(`http://localhost:3000/stories/${id}`, requestOptions)
      .then(response => response.json())
      .then(result => {
        console.log('Story detail data:', result);
        if (result && result.id) {
          callback(result);
        } else {
          console.error('Không thể lấy thông tin user story:', result && result.message ? result.message : 'Lỗi không xác định');
          callback(null);
        }
      })
      .catch(error => {
    ...
    fetch('http://localhost:3000/stories', requestOptions)
      ...
      .then(result => {
        ...
        if (result && result.id) {
          ...
          callback(null, result && result.message ? result.message : 'Không thể tạo user story mới');
        }
      })
      .catch(error => {
    ...
    fetch(`http://localhost:3000/stories/${id}`, requestOptions)
      ...
      .then(result => {
        ...
        if (result && result.id) {
          ...
          callback(null, result && result.message ? result.message : 'Không thể cập nhật user story');
        }
      })
      .catch(error => {
  ...
  deleteStory: function(id, callback) {
    ...
    fetch(`http://localhost:3000/stories/${id}`, requestOptions)
      ...
      .then(result => {
        if (!result) return; // Đã xử lý thành công ở trên
        console.log('Kết quả xóa user story:', result);
        callback(false, result && result.message ? result.message : 'Không thể xóa user story');
      })
      .catch(error => {
