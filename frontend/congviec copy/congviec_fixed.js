...
window.taskAPI = {
  ...
  getAllTasks: function(callback) {
    ...
    fetch('http://localhost:3000/tasks', requestOptions)
      ...
      .then(result => {
        ...
        // Kiểm tra nếu result là mảng (API mới) hoặc có cấu trúc cũ
  ...
  getTaskById: function(id, callback) {
    ...
    fetch(`http://localhost:3000/tasks/${id}`, requestOptions)
      ...
      .then(result => {
        ...
        if (result && (result.id || (result.statusCode === 200 && result.data))) {
          callback(result.id ? result : result.data);
        } else {
  ...
  createTask: function(taskData, callback) {
    ...
    fetch('http://localhost:3000/tasks', requestOptions)
      ...
      .then(result => {
        ...
        // Kiểm tra nếu dữ liệu mới hoặc cũ
        if (result && result.id) {
  ...
  deleteTask: function(id, callback) {
    ...
    fetch(`http://localhost:3000/tasks/${id}`, requestOptions)
      .then(response => {
        if (response.ok) {
          // Trường hợp API chỉ trả về status code thành công mà không có body
          console.log('Xóa nhiệm vụ thành công');
          callback(true, null);
          return null; // Không cần phải parse JSON
        }
...
function displayTasks(tasks) {
  ...
  if (!Array.isArray(tasks)) {
    console.error('Dữ liệu nhiệm vụ không phải là mảng:', tasks);
