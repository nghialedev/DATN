...
export default function Register() {
  const register = {
    login: "Đăng nhập", link_login: "/auth/login", register: "Register", register_link: "/", agree_text: "Tôi đồng ý với các điều khoản và điều kiện", or: "Hoặc", already: "Đã có tài khoản?", join_fb: "Join with FB", join_twitter: "Join with Twitter", footer_copyright: "Tailnet | All right reserved"
  }
  // const options = [
  ...
  return (
    <>
      <Heading variant="h3" className="text-center">Đăng ký</Heading>
      <hr className="block w-12 h-0.5 mx-auto my-5 bg-gray-700 border-gray-700" />
      <form>
        <InputLabel label="Họ và tên" id="input1" name="fullname" />
        <InputLabel type="email" label="Email" id="input2" name="email" />
        <InputPassword type="password" label="Mật khẩu" id="input3" name="password" />
        <InputPassword type="password" label="Nhắc lại mật khẩu" id="input4" name="repassword" />
        {/* <Radio options={options} name="music" checked="rock" label="Music" /> */}
        <div className="mb-6">
          <label className="flex flex-row items-center">
            <Checkbox name="tos" value="1" required="true"/>
            <p className="ml-2">Tôi đồng ý với <a href="#">các điều khoản và điều kiện</a></p>
          </label>
        </div>

        <div className="grid">
          <Button type="submit">
            <BoxArrowInRight className="inline-block w-4 h-4 ltr:mr-2 rtl:ml-2" />Đăng ký
          </Button>
        </div>
      </form>
